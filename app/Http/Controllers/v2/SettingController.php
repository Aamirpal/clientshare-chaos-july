...
use App\Http\Controllers\{ManageShareController, FeedbackController, Controller, UserController};
...
class SettingController extends Controller {
  ...
  use FileHandling, AwsS3, Feedback, Generic;
    ...
    function __construct(SpaceUserInterface $space_user_interface, SpaceInterface $space_interface)  {
      ...
      $this->space_user = $space_user_interface;
      $this->space = $space_interface;
    }
    ...
    public function powerReports($space_id) {
      ...
      $is_logged_in_user_admin = $this->space_user->isAdmin($space_id, \Auth::user()->id);
      return view('v2-views/setting/power_bi', compact('report_data', 'space_id', 'is_logged_in_user_admin'));
    }
    ...
    public function index(Request $request, $space_id, $method=null, $page=null) {
      ...
      if (! isset($user_setting['user_type_id'])) {
            return redirect('/');
        }
      ...
      $data = [
          's3_form_details'=>$s3_form_details,
          'show_tab'=>$show_tab, 
          'feedback_status'=>$feedback_status,
          'domain_management'=>$domain_management, 
          'user_setting'=>$user_setting,
          'feedback_status_to_date' => $feedback_status_to_date,
          'checBuyer' => $check_buyer,
          'space_data'=>$space_with_buyer_seller, 
          'space_id'=>$space_id,
          'is_logged_in_user_admin'=>$this->space_user->isAdmin($space_id, \Auth::user()->id),
          'space_twitter'=>$this->space->getTwitterHandler($space_id)
        ];
        return view('v2-views/setting/index', $data);
    }
    ...
    public function userManagement(Request $request) {
      ...
      if($space_id){
        ...
        return view('v2-views/setting/user_management', [
    ...
    public function pendingInvites(Request $request) {
           ...
           if(!isset($invited_by_users)) $invited_by_users = [];
                return view('v2-views/setting/pending_invites', ['pending_invitations_pagination'=>$pending_invitations,'pending_invitations'=> $invited_by_users])->render();
           }
           return false;
    }
    ...
    public function allowPosting(Request $request_data){
      ...
      $settings_updated = Space::updateSpaceById($request_data->space_id, ['allow_seller_post'=>$allow_seller_post, 'allow_buyer_post'=> $allow_buyer_post,'invite_permission'=>$allow_invite_permission]);
      if (!$settings_updated) {
      ...
      return apiResponseComposer(200, ['success' => 'Permissions updated successfully.'], []);;
    }
    ...
    public function deleteAllowedDomain(Request $request) {
        $space_meta = Space::select('metadata')->where('id', $request->space_id)->first();
        if (!sizeOfCustom($space_meta) || !checkUuidFormat($request->space_id) || trim($request->domain) == explode('@', Auth::user()->email)[1]) {
            return ['success' => false, 'message' => 'This Domain can not be deleted'];
        }
        $updated_domains = [];
        foreach ($space_meta->metadata['rule'] as $domain) {
            if (trim($domain['value']) != trim($request->domain) || trim($domain['value']) == explode('@', Auth::user()->email)[1]) {
                $updated_domains[] = $domain;
            }
        }
        if (!sizeOfCustom($updated_domains)) {
            return ['success' => false, 'message' => 'There should be atleast one domain'];
        }
        if (Space::updateSpaceById($request->space_id, ['metadata' => json_encode(['rule' => $updated_domains])])) {
