...
class LoginController extends Controller {
    ...
    public function __construct() {
       $this->middleware('guest', ['except' => ['logout', 'verifyauthcode']]);
    }
    ...
    public function verifyUser(Request $request){
      ...
      $this->validate($request, [
        ...
        $space_user->save();
        (new MailerController)->registrationVerification($request->all()['email'], $code);
    ...
    if ($this->guard()->attempt($credentials, $request->has('remember'))) {
        ...
        $code = $two_factor_authentication->getCode($secret);
           ...
           $data['auth_code'] = $secret;
            Session::put('verification_code_info', $data); 
            Session::put('verification_in_process',time());
   ...
   */
    protected function hasTooManyLoginAttempts(Request $request){
      return $this->limiter()->tooManyAttempts(
        $this->throttleKey($request), env("LOGIN_ATTEMPTS"),env("LOGIN_FREEZE_TIME") 
        );
    }
   /**
  ...
  public function sendLoginIfTokenEmpty($request){
      ...
      $login_user = Auth::user()->user_type_id;
      $user_type = UserType::find($login_user);
      if(!sizeOfCustom($space_user) && sizeOfCustom($space_user_with_trash) && ($user_type['user_type_name'] != 'super_admin') ){
            $admin_emails = array();
            foreach ($space_user_with_trash as $value) {
              $space_admin_info = SpaceUser::getSpaceAdminInfo($value['space_id'],$trashed='l');if(sizeOfCustom($space_admin_info) < 0 && sizeOfCustom($space_admin_info) == 1 ) {
            ...
            if(isset($admin_emails) && sizeOfCustom($admin_emails)){
             Auth::logout();
            ...
            if( !sizeOfCustom($space_user) && ($user_type['user_type_name'] != 'super_admin') ) {
              Auth::logout();
  ...
  public function verifyauthcode(Request $request){
    ...
    $result = $two_factor_authentication->verifyCode(Session::get('verification_code_info')['auth_code'], trim($request->all()['code']));
    if($result == true){
      ...
      return $this->authenticated($request, $this->guard()->user())
        ...
        Session::put('verification_code_error_count', $data); 
