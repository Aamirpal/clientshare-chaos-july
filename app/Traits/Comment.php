...
use App\{Comment as CommentModel, Notification, Post, Space, SpaceUser, User, UserTaggingComment, EndorsePost};
...
trait Comment {
	...
	public function sendAlerts($comment, $current_user) {
		...
		$people_reacted = (new User)->getPeopleReacted($post->getAllReactedUsers($comment->post_id , [$current_user->id]));
	...
	private function sendTaggedAlert($alert_data, $tagged_users){
		$tagged_users = SpaceUser::checkTagAlertSetting($alert_data['post']['space_id'], $tagged_users, true, in_array(config('constants.USER_ID_DEFAULT'), $tagged_users));
		foreach ($tagged_users as $tagged_user ) {
		  if($tagged_user['user_id'] != $alert_data['current_user']['id']){
            ...
            $alert_data['people_reacted'] = (new User)->getPeopleReacted((new Post)->getAllReactedUsers($alert_data['post']['id'], [$alert_data['current_user']['id'], $alert_data['user']['id']]));
