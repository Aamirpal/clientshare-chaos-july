...
var triggerPendingOnboardingSteps = once(function() {
    if($('.step-uncompleted').length){
        $('.step-uncompleted').eq(0).parent().trigger('click');
    }
});
...
function saveCategoryRequest(categories) {
    ...
    $.ajax({
        ...
        complete: function(){ 
            ...
            $('.category_step').delay(10000).removeClass('step-uncompleted');
            category_flag = true;
        }
    });
...
function updateTourStep(step){
    ...
    $.ajax({
        type: 'GET',
        url: baseurl + '/update-tour-step/' + session_space_id + '/' + (parseInt(step) + 1),
        success: function(response) { 
            if (response.share_profile_completed) {
                location.reload();
            }else{
               return true; 
            }
...
function runOnBoardingTour(force_trigger) {
    ...
    if(share_setup_steps <= 7 || force_trigger)
        if(!($("#user_profile").data('bs.modal') || {})._isShown){
            $('.tour-trigger').trigger('click');
        }
    else if(isMobileDevice() && share_setup_steps <= 7)
        runTour();
}
...
function saveDomainRequest(form_class, current_step) {
    ...
    var domain_array =$(form_to_submit).serializeArray();
       if(domain_array[0]['name']=="restrict_domain"){
           domain_array.splice(0,1);
       }
    domain_array.forEach(function (domain, index) {
        if (domain.value === ''){
            domain_array.splice(index);
        }  
    });
...
function saveDomain(current_step){
    if(!current_step.find('form.save-domain').length)
        return true;
    if(typeof($("input[name='restrict_domain']:checked").val())=='undefined'){
         $("#restrict_domain-radio-error").text("Please select your choice.");
         return false;
    }
    if($("input[name='restrict_domain']:checked").val() != '1'){
        retrictDomin(current_step, false);
    }else{
        $(".tour-domain-list  div input[name='rule']").val("");
...
function displayPanel() {
    pending_steps = $('.step-uncompleted');
...
function addProgressBarsection(refrence){
    ...
    $.ajax({
        type: 'GET',
        url: baseurl+'/share-profile-status?space_id='+session_space_id,
        success: function (response) {
            ...
            if(response.data.progress == parseInt(100) || response.data.space_users) {
                ...
                $(document).find('#feed').removeClass('.onboarding-design-fix');
                return false;
            }
            if(response.result){
...
$(document).on('click', '.admin-progress-icon div', function(){
    $('div.welcome-cs-popup').addClass('hidden');
...
$(document).on('click', '.wc-step-col div', function(){
    $('div.welcome-cs-popup').addClass('hidden');
...
$(document).on('click', '.add-admin-invite-link', function(event){ 
    var html = '<div class="twitter-input-col form-group"><input class="form-control twitter-feed-input" name="admin_invite[]" placeholder="Email address" type="text"> <span class="cross-admin-email remove-admin-email">\n\
<img src="'+baseurl+'/images/ic_delete_hover.svg"></span></div>';
    $('.admin-invite-box').append(html);
});
$(document).on('click','.cross-admin-email',function(){
    $(this).parents('.twitter-input-col.form-group').remove();
});
$(document).on('change', '.radio-invite-admin', function(event){
     $('.validate-invite-admin').html('');
    if($(this).val() === '1'){
        $('.invite-admin-block').show();
        $('.btn-invite-handle').text('Invite')
    }else{
        $('.invite-admin-block').hide();
        $('.btn-invite-handle').text('Next')
    }
});
$(document).on('click', '.btn-invite-handle', function(event){
    current_step = $(this).closest('.welcome-cs-popup');
    if(typeof($("input[name='radio_add_admin']:checked").val()) == 'undefined'){
            $('.validate-invite-admin').html('<span class="admin-invite-error">Please select your choice.</span>');
            return false;
    }else{
        $('.validate-invite-admin').html('');
    }
    
    if( $("input[name='radio_add_admin']:checked").val() !== '1'){
        $.ajax({
            type: "POST",
            headers: {
            "cache-control": "no-cache",
            ...
            url: baseurl + '/update-invite-admin-status/' + current_space_id,
            data: {invite_admin_status: 'true'},
            dataType: 'JSON',
            success: function (response) {
                if (response) {
                    updateNextStep(current_step);
            ...
            error: function (xhr, status, error) {
                logErrorOnPage(xhr, status, error, 'onboarding_invite_admin');
            }
        }); 
        return false;
    }
...
$(document).on('change', '.restrict-domain', function(event){
    $("#restrict_domain-radio-error").text("");
   if($("input[name='restrict_domain']:checked").val() !== '1'){
     $('.add-restrict-domain-wrap').hide(); 
     $('.slider-toggle-off-content ').show();
 }else{
     $('.add-restrict-domain-wrap').show();   
     $('.slider-toggle-off-content ').hide();
 }
});
...
$(document).on('click', '.add-domain', function(){
    ...
    domain_layout_clone.append('<span class="wc-domain-delete"><img src="'+baseurl+'/images/v2-images/close_bg_icon.svg"></span>');
...
$(document).on('click', '.add_post_trigger', function(){
    ...
    document.getElementById('add_post_trigger').click();
});
...
$(document).on('click', 'form#update_welcome_share_banner .onboarding_company_logo', function(){
    ...
    if(seller_twitter_name && buyer_twitter_name)
    {
        ...
        if($('#twitter_handle_1').val() != '' && typeof twitter_handle_last == 'undefined')
...
function saveTwitter(form){
     $('.twitter_links_error').remove();
      ...
      $.ajax({
              ...
              success: function (response) {
                 if(response.result){ 
                    ...
                    $(document).find('.twitter_links_error').remove();
                    ...
                    .append($('<span class="error-msg twitter_links_error">'+response.error+'</span>'));
                 } 
              },
              error: function (xhr, status, error) {
...
$(document).on('submit','#onboarding_twitter_handles',function(e){
       e.preventDefault();
       var form =$(this);
        if ($('#prefer_twitter').prop("checked")) {
           $("input[name='twitter_handles[]']").val("");
           $(this).find('div.twitter-handle').find('span.error').remove();
           saveTwitter(form);
           return false;
        } 
