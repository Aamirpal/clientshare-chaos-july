...
var get_all_share_notifications;
...
function limitText(string, limit){
  if(string.length>limit)
        string = string.replace('/^(.{'+limit+'}[^\s]*).*/', "$1")+'...';
    if(string.length>=limit)
        string = string.substr(0,limit)+'...';
    return string;
}

function stringDateFormat(date_string, format){
  return moment(date_string).format(format);
}
function errorOnPage(xhr, status, error){
  logErrorOnPage(xhr, status, error, 'Error: Please refresh the page [via generic file]');
}
...
function feedSignedUrl(url, element, post_id){
  $.ajax({
    type: "GET",
    url: baseurl + '/url_validate?q=' + url,
    success: function (response) {
      if(response['cloud'] === '' ){
         $('.findmedia .'+element).closest('.media_video_section').append('<p>This file type is being converted and may take a few seconds to appear.</p>');
         $('.findmedia .'+element).closest('video').remove();
         setTimeout( function(){ 
            refreshPost(post_id, {'comment_limit':0});
          }  , 15000 );
      }
      $('.findmedia .'+element).attr('src', response['cloud']);
      $('.findmedia .'+element).closest('video').load();
    }
  });
}

function signedUrl(url, loader, file_name){
  ...
  $.ajax({
    type: "GET",
    async: false,
    url: baseurl + '/url_validate?q=' + url + '&file_name=' + encodeURIComponent(file_name),
    beforeSend:function(request){
      if(loader)
        loader.show();
    },
    success: function (response) {
      signed_Url = validateSignedUrl(response);
    },
    complete:function(xhr,status){
      ...
      if(xhr.status !== 200){
        signed_Url = throwS3AccessDenied();
      }
    }
  });
  return signed_Url;
...
function validateSignedUrl(response){
  if(response.length === 0 || typeof response == 'undefined'){
   return throwS3AccessDenied();
  }
  return response;
}

function throwS3AccessDenied(){
   var response = {'file_ext':'html', 'file_url':'http://clientshare-docs.s3-website-eu-west-1.amazonaws.com/cs_error.html', 'cloud':'http://clientshare-docs.s3-website-eu-west-1.amazonaws.com/cs_error.html'};
   return response;
}
...
function extractUrlParam(url, param){
  ...
  var url_parameters = query[1].split("&");
  for (var i=0;i<url_parameters.length;i++) {
    var pair = url_parameters[i].split("=");
    if (pair[0] == param) {
      return pair[1];
...
function liked_info(element){
  ...
  $.ajax({
    ...
    error: function(xhr, status, error) {
      ...
      logErrorOnPage(xhr, status, error, 'Linked info');
    }
  });
}
...
function setLinkedinSession(){
  ...
  var job_title = $('#update_profile_form #user_job_title').val();
  var biotext = $('#update_profile_form #user_bio').val();
  var linkedin_link = $('#update_profile_form #user_linkedin').val();
  if( $('#update_profile_form #user_phone_number').length ) {
   var phone_no = $('#update_profile_form #user_phone_number').val();
  }else{
   var phone_no = '';
  }  
  ...
  $(document).on('keypress', '.invite_email_inp, input[name=email]', function(e) {
    ...
    form_s3.fileupload({  
      ...
      add: function (event, data) {
        ...
        if (direct_upload_s3_data[0]['allowed_extension'].indexOf((file.name.split('.').pop().toLowerCase())) < 0) {
        ...
        if (direct_upload_s3_data[0]['progress_bar_ele']) {
          $(direct_upload_s3_data[0]['progress_bar_ele']).append('<div class="single-post-loading"><div style="width: 0;" onclick="close_preview_general_s3(this);" id="' + uid + '" class="s3_running_process edit-load-process ' + uid + '"><span class="small-del"><img src="' + baseurl + '/images/v2-images/close-icon.svg' + '"></span><div class="progress small"><div class="progress-bar" role="progressbar" aria-valuenow="" aria-valuemin="0" aria-valuemax="" style="width:100%"><span class="sr-only"></span></div></div><span class="close"></div></div>');
        }
      },
      progress: function (e, data) {
        $('.save_executive_btn').prop('disabled', true);
        $('.onboarding_save_executive_btn').prop('disabled', true);
...
function setS3FormData(s3_form, s3_form_details){
  s3_form.attr('action', s3_form_details.url);
  var form_inputs = '';
  $.each(s3_form_details.inputs , function(input_name, input_value){
...
$(document).find(".notification-dropdown.dropdown").on("show.bs.dropdown", function (event) {
    $.ajax({
        type: "GET",
        url: baseurl + '/reset-notifications-badge/' + current_space_id+'/'+loggin_user_id,
        success: function (response) {
            if(response.data){
                $(document).find('.notification_count').html('');
            }
        }
    });
});
