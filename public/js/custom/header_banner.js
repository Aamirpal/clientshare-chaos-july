...
function checkValidTwitterHandle(handle_value){
...
$('document').ready(function(){
      ...
      $.ajax({
        ...
        success: function(response) {
            if(response.result){
              ...
              $('.welcome_share_wrap .space-pic-wrap span:first-child').css({"background-image":"url('"+ response.seller_logo +"')"});

              if(response.buyer_logo.trim())
              $('.welcome_share_wrap .space-pic-wrap span:nth-child(2)').css({"background-image":"url('"+ response.buyer_logo +"')"});

              if(response.background_image.trim())
              $('.main-banner-row .container-fluid').css({"background-image":"url('"+ response.background_image +"')"});
              ...
              addProgressBarsection();
                ...
                $('.'+response.key).addClass('has-error');
                $('.'+response.key).find('.twitter-error').remove();
            }
        },
        error: function(xhr, status, error){
  ...
  $('form#edit_share_header .twitter-feed-input,form#update_welcome_share_logo .twitter-feed-input').on('change', function(){
      ...
      if(twitter_handle != '' && typeof twitter_handle != 'undefined'){
        ...
        if(twitter_handle.indexOf('@') == -1) twitter_handle = '@'+twitter_handle;
        setTimeout(function(){
            if(checkValidTwitterHandle(twitter_handle)){
  ...
  $.ajax({
        ...
        success: function(response) {
            if(response.result){
              ...
              $('.twitter-error').remove();
              $(logo_edit_block).find('.twitter-input-col').append('<div class="email-error image-error twitter-error">'+response.error+'</div>');
            }
        },
        error: function(xhr, status, error){
